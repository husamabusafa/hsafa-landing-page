...
import { IconPalette, IconChevronDown, IconTrash, IconMaximize, IconFileText, IconCode, IconDeviceFloppy } from '@tabler/icons-react';
...
interface CustomUIComponentNodeData {
  label?: string;
  color?: string;
  agentColor?: string;
  title?: string;
...
export default function CustomUIComponentNode({ data, selected, id }: NodeProps) {
  ...
  const nodeData = (data as CustomUIComponentNodeData) || {};
  ...
  const label = TT('ui_components', lang);
  ...
  }, [nodeData.title, nodeData.description, nodeData.zodSchema, nodeData.isExpanded]);
  ...
  }, [title, description, zodSchema, isFlowReady, id, nodeData.title, nodeData.description, nodeData.zodSchema, nodeData.isConfigured, isExpanded, updateNodeData]);
  ...
  return (
    <>
      ...
      >
        ...
        <div className={`relative flex items-center justify-between p-6 gap-4 ${!isExpanded && !isConfigured ? 'pb-6' : 'pb-4'}`}>
          <div className="flex items-center gap-4">
            ...
            >
              <IconPalette size={24} className="drop-shadow-lg transition-all duration-300" />
            </div>
            <div>
            ...
            <div className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold border-2 transition-all duration-300 ${
              ...
              {isConfigured ? (
                <>
                  ...
                  {TT('ui_component_configured', lang)}
                </>
              ) : (
                <>
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={TT('ui_component_editor', lang)}
        size="2xl"
        maxHeight={true}
      >
